...
function get_leftmost_indep_columns(m)
    ...
    while true
        ...
        r1 = rank(mr[:,idx[1:length(ondiag)][ondiag]])
        println("rank: ",r1)
        if r1==r 
...
plot(J,st=:scatter,msw=0,color=colors,ms=2,legend=:none,xlabel="Clusters",ylabel="ECI size")
plot(abs.(m*J-en))
norm(m*J-en)
c2 = colors[idx]
...
plot!(sizes, errVal,yaxis=:log,st=:scatter,msw=0,ms=2,xlabel="Model size (num parameters)",ylabel="Avg Val Error",legend=:none,color=colors[sizes])
